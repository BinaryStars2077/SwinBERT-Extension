...
class VideoTransformer(torch.nn.Module):
    ...
    def generate_mask(self):
        t, h, w = 16, 7, 7
        tm, hm, wm = 3, 4, 4
        mask = torch.ones(t, h, w, t, h, w)
        """
        for tdx in range(t):
            for idx_h1 in range(h):
                for idx_w1 in range(w):
                    idx_start = max(tdx - tm // 2, 0)
                    idx_end = min(tdx + tm // 2 + 1, h)
                    idx_h2_start = max(idx_h1 - hm // 2, 0)
                    idx_h2_end = min(idx_h1 + hm // 2 + 1, h)
                    idx_w2_start = max(idx_w1 - wm // 2, 0)
                    idx_w2_end = min(idx_w1 + wm // 2 + 1, w)
                    mask[
                    tdx, idx_h1, idx_w1, idx_start:idx_end, idx_h2_start:idx_h2_end, idx_w2_start:idx_w2_end
                    ] = 0
                    mask[
                        tdx, idx_h1, idx_w1, tdx, idx_h1, idx_w1
                    ] = 1
        """
        mask = mask.view(t * h * w, t * h * w)
        mask = (
            mask.float()
                .masked_fill(mask == 0, float(0.0))
                .masked_fill(mask == 1, float(1.0))
                .cuda()
        )
        return mask
    def forward(self, *args, **kwargs):
        ...
        if self.learn_mask_enabled:
            ...
            print(((learn_att==1.0).sum()-16*7*7)/(784*784))
            kwargs['attention_mask'][:, -vid_att_len::, -vid_att_len::] = self.generate_mask()
        outputs = self.trans_encoder(*args, **kwargs)
        if self.learn_mask_enabled:#set false
